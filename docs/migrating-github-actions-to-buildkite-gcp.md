...
| GCS hook path mismatch after terraform | `Repository not found` on clone — module's built-in hook runs instead of custom | Packer startup script downloads from `gs://BUCKET/environment` (root), but terraform may upload to `hooks/environment`. Upload to both paths — the startup script path and the self-refresh path. |
| Terraform destroys secrets on apply | `secret_data` replaced with placeholder — breaks API token, SSH keys | Add `lifecycle { ignore_changes = [secret_data] }` to ALL `google_secret_manager_secret_version` resources. Without it, running `terraform apply` with missing/placeholder env vars overwrites production secrets. |
| Deploy SSH key `error in libcrypto` | `Load key: error in libcrypto` + `Permission denied (publickey)` | Secret was overwritten by terraform (see above). Restore from local key: `gcloud secrets versions add deploy-ssh-key --data-file=~/.ssh/auxscan-nginx-deploy`. Always verify with `gcloud secrets versions access latest --secret=deploy-ssh-key \| head -1` — should show `-----BEGIN OPENSSH PRIVATE KEY-----`. |
| Deploy target IP stale after server migration | Deploy SSHs to old server, times out or `Permission denied` | `deploy-target-*` secrets in Secret Manager may contain old IPs from initial setup. When a server migrates, update both the secret (`gcloud secrets versions add deploy-target-NAME --data-file=-`) and `terraform.tfvars`. GHA had IPs in its own secrets — migrating to Buildkite doesn't automatically carry them over. |
| Placeholder TF vars cascade into secret destruction | Multiple secrets destroyed in one `terraform apply` | Never pass placeholder values for sensitive vars. Use `ignore_changes` on all secret versions AND verify secrets after apply: `gcloud secrets versions access latest --secret=NAME \| head -1`. The `post-apply-checks.sh` script catches hook issues but not secret content — consider extending it. |
| Buildkite pipeline branch filter blocks PRs | Feature/PR branches don't trigger builds | Pipeline Settings → Build Branches must be `*` (all), not just `main staging development`. Webhook delivers (200) but no build created. |
| Cross-project IAM for scan VMs | `User does not have access to service account` when creating VMs in another project | Buildkite agent SA needs `iam.serviceAccountUser` on the target SA, plus `compute.instanceAdmin.v1`, `artifactregistry.writer`, and `secretmanager.secretAccessor` in the target project. |
| GitHub App not installed on repo | `publish_commit_status: true` but no statuses on GitHub | Webhook-only connection can trigger builds but can't post statuses. Install Buildkite GitHub App on the repo via GitHub → Settings → Installations. May need to push a new commit after installation for statuses to start appearing. |
| VM metadata missing for runtime scripts | Slack shutdown notification doesn't fire from idle-shutdown.sh | Scripts running on the VM can only access metadata, not local `.env`. Set `SLACK_WEBHOOK_URL` via `gcloud compute instances add-metadata`. `vm-create.sh` should pass it at creation time. |
| `docker.io` over Docker CE in Packer image | `docker.sock` EOF errors, exit 143 mid-container | Base Buildkite image has Docker CE. Packer template was installing `docker.io` on top, corrupting the daemon. Guard with `command -v docker` — only install if missing. The startup-script.sh fix was irrelevant because elastic stack uses the Packer image, not the startup script. |
| Packer image not picked up after rebuild | `terraform apply` creates template with old image | Image name is a variable, not a family lookup. After `packer build`, update `agent_image` in `terraform.tfvars` then `terraform apply`. Remove defaults from image variables to force explicit specification — stale defaults silently use old images. |
| N2D CPU quota limits VM count | Scaler targets 8 VMs but only 4 create, rest stuck | N2D quota was 16 CPUs (4× n2d-standard-4). Switch to e2-standard-4 — same specs, 27% cheaper on spot, 200 CPU quota (50 VMs). Check quota with `gcloud compute regions describe REGION --format=json \| jq '.quotas[] \| select(.metric=="N2D_CPUS")'`. |
